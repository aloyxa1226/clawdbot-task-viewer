...
○ clawdbot-task-viewer-lla · Dark theme: update CSS tokens   [● P1 · OPEN]
...
## Research Context
- Current CSS uses shadcn/ui pattern with :root (light) and .dark class
- Dark theme tokens already exist in .dark class but are unused
- Hardcoded colors in components (bg-white, text-gray-*) need removal

## Acceptance Criteria (US-010)
- [ ] Background uses dark values (~#0f172a / 222 47% 6%)
...
## Files
- MODIFY: client/src/index.css (replace :root values, add utility classes)
- MODIFY: client/src/components/TaskCard.tsx (remove hardcoded colors)
- MODIFY: Any other components with hardcoded light colors

ACCEPTANCE CRITERIA
- [ ] :root has dark values
- [ ] No bg-white or text-gray-* in codebase
- [ ] WCAG AA contrast met
- [ ] Typecheck passes

BLOCKS
  ← ○ clawdbot-task-viewer-1l4: Header component: consolidated command center ● P1
  ← ○ clawdbot-task-viewer-ed3: KanbanColumn: stats header, droppable area ● P1
  ← ○ clawdbot-task-viewer-j1l: TaskCard: priority pips, timestamps, status styling, hover actions ● P1
  ← ○ clawdbot-task-viewer-eu2: Dialog theming: dark styles for Detail, Edit, Create dialogs ● P2
```
...
- Working directory: /Users/admin/clawd/clawdbot-task-viewer/.worktrees/clawdbot-task-viewer-lla
- Branch: bead/clawdbot-task-viewer-lla
- Bead ID: clawdbot-task-viewer-lla
